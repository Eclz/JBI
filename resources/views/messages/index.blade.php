...
@section('title', 'Student Mailbox & Messaging')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h5 class="fw-bold text-dark text-uppercase mb-0">
                <i class="bi bi-envelope-paper text-primary me-2"></i>STUDENT MAILBOX & MESSAGING
            </h5>
            <p class="text-muted small mb-0">Send messages to fellow students and faculty lecturers, and receive assessment alerts</p>
        </div>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px; overflow: hidden;">
            <form action="{{ route('messages.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-uppercase">Recipient (Faculty Lecturer / Classmate) <span class="text-danger">*</span></label>
                        <select name="receiver_id" class="form-select" required>
                            <option value="">-- Select Recipient --</option>
                            @foreach($recipients as $r)
                                <option value="{{ $r->id }}">{{ $r->first_name }} {{ $r->last_name }} ({{ ucfirst($r->role) }} - {{ $r->email }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="subject" class="form-control" placeholder="e.g. Question regarding SE301 Course Project" required>
                    </div>
