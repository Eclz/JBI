...
@section('title', 'Elected Student Leaders Cabinet')
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1 text-primary fw-bold">
                <i class="bi bi-award me-2"></i>Elected Student Leaders Cabinet
            </h1>
            <p class="text-muted mb-0">Official directory of elected student leaders serving across university-wide and faculty portfolios</p>
        </div>
        <a href="{{ route('admin.evoting.index') }}" class="btn btn-outline-primary fw-semibold">
                ...
                <div class="card border-0 shadow-sm h-100 rounded-3 text-center overflow-hidden">
                    <div class="bg-primary py-4 px-3 text-white position-relative" style="background: linear-gradient(135deg, #1e3a8a, #3b82f6) !important;">
                        <span class="badge bg-warning text-dark position-absolute top-0 end-0 m-2 fw-bold">
                            <i class="bi bi-trophy-fill me-1"></i>Elected Leader
                        </span>
                        @if($leader->photo)
                    ...
                    <div class="card-body p-3 d-flex flex-column justify-content-between flex-grow-1">
                        <div>
                            <div class="badge bg-light text-primary border mb-2 fs-6">
                                {{ $leader->position->title ?? 'Portfolio' }}
                            </div>
                            ...
                            <div class="small text-muted mb-3">
                                <div><strong>Election:</strong> {{ $leader->position?->session?->title ?? 'N/A' }}</div>
                                <div><strong>Academic Year:</strong> {{ $leader->position?->session?->academicYear?->name ?? 'N/A' }}</div>
                            </div>
                            @if($leader->manifesto)
                        ...
                        <div class="border-top pt-2 mt-3 small text-muted">
                            <i class="bi bi-envelope me-1"></i>{{ $leader->user->email ?? 'Contact Student' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm py-5 text-center bg-white rounded-3">
                    <div class="card-body">
                        ...
                        <h4 class="fw-bold text-dark mb-1">No Elected Student Leaders Yet</h4>
                        <p class="text-muted mb-3">Once an election concludes and the Electoral Commission publishes the official results, the winning candidates will automatically appear here as the official Student Leadership Cabinet.</p>
                        <a href="{{ route('admin.evoting.index') }}" class="btn btn-primary fw-bold">
                            <i class="bi bi-check2-square me-1"></i>View Election Seasons
                        </a>
                    </div>
                </div>
            </div>
    ...
    @if($leaders instanceof \Illuminate\Pagination\LengthAwarePaginator && $leaders->hasPages())
