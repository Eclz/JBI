...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted">Update {{ $program->name }}</p>
        </div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.programs.update', $program) }}" method="POST">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('department_id') is-invalid @enderror" id="department_id" name="department_id" required>
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" {{ old('department_id', $program->department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                            @endforeach
                        </select>
                        @error('department_id')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('program_level_id') is-invalid @enderror" id="program_level_id" name="program_level_id">
                            ...
                            @foreach($levels as $level)
                                <option value="{{ $level->id }}" {{ old('program_level_id', $program->program_level_id) == $level->id ? 'selected' : '' }}>{{ $level->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-check mb-4">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" {{ old('is_active', $program->is_active) ? 'checked' : '' }}>
